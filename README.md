# CySec-Learning

Self-study log to faciliate my move into cyber security

I'm a third-semester B.Sc. Information Engineering student at TUM Campus Heilbronn. This repo tracks what I'm learning over the 2026 summer break (and hopefully beyond), mostly Linux and networking/cysec fundamentals.

## Contents

- [`pwn-college/`](./pwn-college) — Notes and write-ups from the pwn.college Linux Luminarium module along with other modules!

---

## pwn.college

### Getting Started
- [ ] Start Here / Introduction to the Dojo
- [x] Linux Luminarium — *2026-08-22*
- [ ] Computing 101
- [ ] Playing with Programs

### Core Material
- [ ] Intro to Cybersecurity
- [ ] Program Security
- [ ] System Security
- [ ] Software Exploitation

### Community Material
- [ ] Art of the Shell
- [ ] Privilege Escalation
- [ ] Linux Lunacy
- [ ] Pwndamentals
- [ ] Web Security

## Networking Fundamentals — Professor Messer (N10-009)
- [ ] Complete the full Network+ video series
	- [ ] 1.0 Networking Concepts
	- [ ] 2.0 Network Implementation
	- [ ] 3.0 Network Operations
	- [ ] 4.0 Network Security
	- [ ] 5.0 Network Troubleshooting
- [ ] Messer practice exams (as knowledge checks)

## CompTIA Security+ (SY0-701)
- [ ] Complete the Professor Messer SY0-701 series
	- [ ] 1.0 General Security Concepts
	- [ ] 2.0 Threats, Vulnerabilities, and Mitigations
	- [ ] 3.0 Security Architecture
	- [ ] 4.0 Security Operations
	- [ ] 5.0 Security Program Management and Oversight
- [ ] Messer practice exams — aim consistent 85%
- [ ] Book and sit the exam (before mid-2027)

## Microsoft SC-900 — Security, Compliance & Identity Fundamentals
- [ ] Microsoft Learn path — SC-900
	- [ ] Concepts of security, compliance, and identity
	- [ ] Microsoft Entra ID capabilities
	- [ ] Microsoft security solutions (Defender, Sentinel overview)
	- [ ] Microsoft compliance solutions (Purview)
- [ ] Practice assessment
- [ ] Book and sit the exam

## Microsoft SC-200 — Security Operations Analyst
- [ ] Microsoft Learn path — SC-200
- [ ] KQL practice — build queries against sample data
- [ ] Book and sit the exam

## SOC / Blue Team
- [ ] TryHackMe — SOC Level 1 path
	- [ ] Cyber Defence Frameworks
	- [ ] Cyber Threat Intelligence
	- [ ] Network Security and Traffic Analysis
	- [ ] Endpoint Security Monitoring
	- [ ] Security Information and Event Management
	- [ ] Digital Forensics and Incident Response
	- [ ] Phishing Analysis
- [ ] Splunk free training
- [ ] Blue Team Labs Online

## German Differentiators
- [ ] ISO 27001 foundation
- [ ] BSI IT-Grundschutz (bsi.bund.de)
	- [ ] IT-Grundschutz-Kompendium — structure and Bausteine
	- [ ] BSI-Standards 200-1, 200-2, 200-3
	- [ ] Schutzbedarfsfeststellung
- [ ] Regulatory awareness
	- [ ] NIS2 — scope and German implementation
	- [ ] KRITIS — sectors and thresholds
	- [ ] DORA — financial sector scope
	- [ ] BSI-Gesetz basics

## Certifications

- [ ] ISC2 Certified in Cybersecurity
- [x] Cisco — Introduction to Cybersecurity
- [ ] **CompTIA Security+ (SY0-701)**
- [ ] **Microsoft SC-900**
- [ ] **Microsoft SC-200** 
- [ ] Splunk Core Certified User
- [ ] BTL1 (Security Blue Team Level 1) 
- [ ] CySA+ *or* ISO 27001 Lead Auditor